...
public final class XMSSMTSignature implements XMSSStoreableObjectInterface, Encodable {
        ...
        if (access$100 != null) {
            ...
            if (access$100.length == ceil + treeDigestSize + (access$000.getLayers() * height)) {
                ...
                if (XMSSUtil.isIndexValid(access$000.getHeight(), bytesToXBigEndian)) {
                    int i11 = ceil + 0;
                    this.random = XMSSUtil.extractBytesAtOffset(access$100, i11, treeDigestSize);
                    this.reducedSignatures = new ArrayList();
                    for (int i12 = i11 + treeDigestSize; i12 < access$100.length; i12 += height) {
                        this.reducedSignatures.add(new XMSSReducedSignature.Builder(this.params.getXMSSParameters()).withReducedSignature(XMSSUtil.extractBytesAtOffset(access$100, i12, height)).build());
                    }
                    return;
                }
    ...
    public byte[] toByteArray() {
        ...
        int i11 = ceil + 0;
        XMSSUtil.copyBytesAtOffset(bArr, this.random, i11);
        int i12 = i11 + treeDigestSize;
        for (XMSSReducedSignature byteArray : this.reducedSignatures) {
            XMSSUtil.copyBytesAtOffset(bArr, byteArray.toByteArray(), i12);
            i12 += height;
        }
        return bArr;
    }

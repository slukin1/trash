...
public class SM2KeyExchange {
    ...
    private BigInteger reduce(BigInteger bigInteger) {
        return bigInteger.and(BigInteger.valueOf(1).shiftLeft(this.f59098w).subtract(BigInteger.valueOf(1))).setBit(this.f59098w);
    }
    ...
    public byte[] calculateKey(int i11, CipherParameters cipherParameters) {
        ...
        byte[] z12 = getZ(this.digest, bArr, sM2KeyExchangePublicParameters.getStaticPublicKey().getQ());
        ECPoint calculateU = calculateU(sM2KeyExchangePublicParameters);
        return this.initiator ? kdf(calculateU, z11, z12, i11) : kdf(calculateU, z12, z11, i11);
    }
    ...
    public byte[][] calculateKeyWithConfirmation(int i11, byte[] bArr, CipherParameters cipherParameters) {
        ...
        if (!this.initiator || bArr3 != null) {
            byte[] z11 = getZ(this.digest, this.userID, this.staticPubPoint);
            byte[] z12 = getZ(this.digest, bArr2, sM2KeyExchangePublicParameters.getStaticPublicKey().getQ());
            ECPoint calculateU = calculateU(sM2KeyExchangePublicParameters);
            if (this.initiator) {
                byte[] kdf = kdf(calculateU, z11, z12, i12);
                byte[] calculateInnerHash = calculateInnerHash(this.digest, calculateU, z11, z12, this.ephemeralPubPoint, sM2KeyExchangePublicParameters.getEphemeralPublicKey().getQ());
            ...
            byte[] kdf2 = kdf(calculateU, z12, z11, i12);
            byte[] calculateInnerHash2 = calculateInnerHash(this.digest, calculateU, z12, z11, sM2KeyExchangePublicParameters.getEphemeralPublicKey().getQ(), this.ephemeralPubPoint);
    ...
    public void init(CipherParameters cipherParameters) {
        ...
        this.f59098w = (this.ecParams.getCurve().getFieldSize() / 2) - 1;
    }
}

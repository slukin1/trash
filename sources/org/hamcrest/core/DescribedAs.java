...
public class DescribedAs<T> extends BaseMatcher<T> {
    ...
    public void describeTo(Description description) {
        ...
        int i11 = 0;
        while (matcher2.find()) {
            description.appendText(this.descriptionTemplate.substring(i11, matcher2.start()));
            description.appendValue(this.values[Integer.parseInt(matcher2.group(1))]);
            i11 = matcher2.end();
        }
        if (i11 < this.descriptionTemplate.length()) {
            description.appendText(this.descriptionTemplate.substring(i11));
        }
    }

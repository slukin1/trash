...
public final class OperatorReplay<T> extends ConnectableObservable<T> implements Subscription {
    public static final Func0 DEFAULT_UNBOUNDED_FACTORY = new Func0() {
        public Object call() {
    ...
    public static final class ReplaySubscriber<T> extends Subscriber<T> {
        ...
        public void makeRequest(long j11, long j12) {
            ...
            if (j14 != 0) {
                ...
                if (producer2 == null) {
                    ...
                    producer2.request(j13 + j14);
                } else {
                    producer2.request(j14);
                }
            } else if (j13 != 0 && producer2 != null) {
                this.maxUpstreamRequested = 0;
                producer2.request(j13);
            }
        }
    ...
    public static final class SizeAndTimeBoundReplayBuffer<T> extends BoundedReplayBuffer<T> {
        ...
        public SizeAndTimeBoundReplayBuffer(int i11, long j11, Scheduler scheduler2) {
    ...
    public static <T> ConnectableObservable<T> create(Observable<? extends T> observable, final int i11) {
        if (i11 == Integer.MAX_VALUE) {
            return create(observable);
        }
        return create(observable, new Func0<ReplayBuffer<T>>() {
            public ReplayBuffer<T> call() {
                return new SizeBoundReplayBuffer(i11);
            }
        });
    }

    public static <T> ConnectableObservable<T> create(Observable<? extends T> observable, long j11, TimeUnit timeUnit, Scheduler scheduler) {
        return create(observable, j11, timeUnit, scheduler, Integer.MAX_VALUE);
    }

    public static <T> ConnectableObservable<T> create(Observable<? extends T> observable, long j11, TimeUnit timeUnit, final Scheduler scheduler, final int i11) {
        final long millis = timeUnit.toMillis(j11);
        return create(observable, new Func0<ReplayBuffer<T>>() {
            public ReplayBuffer<T> call() {
                return new SizeAndTimeBoundReplayBuffer(i11, millis, scheduler);
            }
        });
    }

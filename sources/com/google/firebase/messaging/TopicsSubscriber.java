...
class TopicsSubscriber {
    ...
    public static Task<TopicsSubscriber> createInstance(FirebaseMessaging firebaseMessaging2, Metadata metadata2, GmsRpc gmsRpc, Context context2, ScheduledExecutorService scheduledExecutorService) {
        return Tasks.call(scheduledExecutorService, new b0(context2, scheduledExecutorService, firebaseMessaging2, metadata2, gmsRpc));
    }
    ...
    public boolean performTopicOperation(TopicOperation topicOperation) throws IOException {
        try {
            ...
            if (c11 == 0) {
                ...
                Log.d("FirebaseMessaging", "Unknown topic operation" + topicOperation + InstructionFileId.DOT);
            }
            return true;
    ...
    public void scheduleSyncTaskWithDelaySeconds(Runnable runnable, long j11) {
        this.syncExecutor.schedule(runnable, j11, TimeUnit.SECONDS);
    }
    ...
    public void syncWithDelaySecondsInternal(long j11) {
        scheduleSyncTaskWithDelaySeconds(new TopicsSyncTask(this, this.context, this.metadata, Math.min(Math.max(30, 2 * j11), MAX_DELAY_SEC)), j11);

...
abstract class Atom {
    ...
    public static final class ContainerAtom extends Atom {
        ...
        public int getChildAtomOfTypeCount(int i11) {
            int size = this.leafChildren.size();
            int i12 = 0;
            for (int i13 = 0; i13 < size; i13++) {
                if (this.leafChildren.get(i13).type == i11) {
                    i12++;
                }
            }
            int size2 = this.containerChildren.size();
            for (int i14 = 0; i14 < size2; i14++) {
                if (this.containerChildren.get(i14).type == i11) {
                    i12++;
                }
            }
            return i12;
        ...
        public ContainerAtom getContainerAtomOfType(int i11) {
            int size = this.containerChildren.size();
            for (int i12 = 0; i12 < size; i12++) {
                ContainerAtom containerAtom = this.containerChildren.get(i12);
                if (containerAtom.type == i11) {
        ...
        public LeafAtom getLeafAtomOfType(int i11) {
            int size = this.leafChildren.size();
            for (int i12 = 0; i12 < size; i12++) {
                LeafAtom leafAtom = this.leafChildren.get(i12);
                if (leafAtom.type == i11) {
                    return leafAtom;
                }
            }
            return null;

...
class MonthAdapter extends BaseAdapter {
    ...
    private boolean isSelected(long j11) {
        for (Long longValue : this.dateSelector.getSelectedDays()) {
            if (UtcDates.canonicalYearMonthDay(j11) == UtcDates.canonicalYearMonthDay(longValue.longValue())) {
                return true;
            }
        }
        return false;
    ...
    private void updateSelectedState(TextView textView, long j11) {
        CalendarItemStyle calendarItemStyle;
        if (textView != null) {
            if (this.calendarConstraints.getDateValidator().isValid(j11)) {
                textView.setEnabled(true);
                if (isSelected(j11)) {
                    calendarItemStyle = this.calendarStyle.selectedDay;
                } else if (UtcDates.getTodayCalendar().getTimeInMillis() == j11) {
    ...
    private void updateSelectedStateForDate(MaterialCalendarGridView materialCalendarGridView, long j11) {
        if (Month.create(j11).equals(this.month)) {
            updateSelectedState((TextView) materialCalendarGridView.getChildAt(materialCalendarGridView.getAdapter().dayToPosition(this.month.getDayOfMonth(j11)) - materialCalendarGridView.getFirstVisiblePosition()), j11);
        }
    }

    public int dayToPosition(int i11) {
        return firstPositionInMonth() + (i11 - 1);
    }
    ...
    public long getItemId(int i11) {
        return (long) (i11 / this.month.daysInWeek);
    }
    ...
    public boolean isFirstInRow(int i11) {
        return i11 % this.month.daysInWeek == 0;
    }

    public boolean isLastInRow(int i11) {
        return (i11 + 1) % this.month.daysInWeek == 0;
    }
    ...
    public int positionToDay(int i11) {
        return (i11 - this.month.daysFromStartOfWeekToFirstOfMonth()) + 1;
    }
    ...
    public boolean withinMonth(int i11) {
        return i11 >= firstPositionInMonth() && i11 <= lastPositionInMonth();
    }

    public Long getItem(int i11) {
        if (i11 < this.month.daysFromStartOfWeekToFirstOfMonth() || i11 > lastPositionInMonth()) {
            return null;
        }
        return Long.valueOf(this.month.getDay(positionToDay(i11)));
    }

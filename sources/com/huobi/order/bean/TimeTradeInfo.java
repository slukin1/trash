...
public class TimeTradeInfo implements Serializable, s9.a {
    private a callback;
    ...
    public TimeTradeInfo(AlgoOrderInfo algoOrderInfo) {
        this.orderInfo = algoOrderInfo;
    }

    public boolean canEqual(Object obj) {
        return obj instanceof TimeTradeInfo;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof TimeTradeInfo)) {
            return false;
        }
        TimeTradeInfo timeTradeInfo = (TimeTradeInfo) obj;
        if (!timeTradeInfo.canEqual(this)) {
            return false;
        }
        AlgoOrderInfo orderInfo2 = getOrderInfo();
        AlgoOrderInfo orderInfo3 = timeTradeInfo.getOrderInfo();
        if (orderInfo2 != null ? !orderInfo2.equals(orderInfo3) : orderInfo3 != null) {
            return false;
        }
        if (isTrade() != timeTradeInfo.isTrade()) {
            return false;
        }
        a callback2 = getCallback();
        a callback3 = timeTradeInfo.getCallback();
        if (callback2 != null ? !callback2.equals(callback3) : callback3 != null) {
            return false;
        }
        TradeType tradeType2 = getTradeType();
        TradeType tradeType3 = timeTradeInfo.getTradeType();
    ...
    public String getViewHandlerName() {
        return TimeOrderHandler.class.getName();
    }
    ...
    public String toString() {
        return "TimeTradeInfo(orderInfo=" + getOrderInfo() + ", isTrade=" + isTrade() + ", callback=" + getCallback() + ", tradeType=" + getTradeType() + ")";
    }

    public TimeTradeInfo(AlgoOrderInfo algoOrderInfo, boolean z11) {
        this.orderInfo = algoOrderInfo;
        this.isTrade = z11;
    }
}

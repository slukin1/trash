...
public class Database implements ChromeDevtoolsDomain {
    ...
    public static class DatabasePeerRegistrationListener extends PeersRegisteredListener {
        ...
        public synchronized void onPeerAdded(JsonRpcPeer jsonRpcPeer) {
            ...
            for (int i11 = 0; i11 < size; i11++) {
                int keyAt = this.mDatabaseHolders.keyAt(i11);
                DatabaseDescriptorHolder valueAt = this.mDatabaseHolders.valueAt(i11);
                DatabaseObject databaseObject = new DatabaseObject();
                databaseObject.f65006id = String.valueOf(keyAt);
    ...
    public static ArrayList<String> flattenRows(Cursor cursor, int i11) {
        Util.throwIfNot(i11 >= 0);
        ArrayList<String> arrayList = new ArrayList<>();
        int columnCount = cursor.getColumnCount();
        for (int i12 = 0; i12 < i11 && cursor.moveToNext(); i12++) {
            for (int i13 = 0; i13 < columnCount; i13++) {
                int type = cursor.getType(i13);
                if (type == 0) {
                    arrayList.add((Object) null);
                } else if (type == 1) {
                    arrayList.add(String.valueOf(cursor.getLong(i13)));
                } else if (type == 2) {
                    arrayList.add(String.valueOf(cursor.getDouble(i13)));
                } else if (type != 4) {
                    arrayList.add(cursor.getString(i13));
                } else {
                    arrayList.add(blobToString(cursor.getBlob(i13)));
                }
            }
        }
    ...
    public JsonRpcResult executeSQL(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) {
        ...
        try {
            return databaseDescriptorHolder.driver.executeSQL(databaseDescriptorHolder.descriptor, executeSQLRequest.query, new BaseDatabaseDriver.ExecuteResultHandler<ExecuteSQLResponse>() {
                public ExecuteSQLResponse handleInsert(long j11) throws SQLiteException {
                    ...
                    executeSQLResponse.values = Collections.singletonList(String.valueOf(j11));
                ...
                public ExecuteSQLResponse handleUpdateDelete(int i11) throws SQLiteException {
                    ExecuteSQLResponse executeSQLResponse = new ExecuteSQLResponse();
                    executeSQLResponse.columnNames = Collections.singletonList("Modified rows");
                    executeSQLResponse.values = Collections.singletonList(String.valueOf(i11));
                    return executeSQLResponse;
                }
            });
        } catch (RuntimeException e11) {
            LogUtil.e(e11, "Exception executing: %s", executeSQLRequest.query);
    ...
    public JsonRpcResult getDatabaseTableNames(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) throws JsonRpcException {
        ...
        try {
            ...
            throw new JsonRpcException(new JsonRpcError(JsonRpcError.ErrorCode.INVALID_REQUEST, e11.toString(), (JSONObject) null));
        }
    }
}

...
public class OAuth1aService extends OAuthService {
    ...
    public OAuthApi api = ((OAuthApi) getRetrofit().create(OAuthApi.class));
    ...
    public String buildCallbackUrl(TwitterAuthConfig twitterAuthConfig) {
        return Uri.parse(CALLBACK_URL).buildUpon().appendQueryParameter("version", getTwitterCore().getVersion()).appendQueryParameter(a.f30241h, twitterAuthConfig.getConsumerKey()).build().toString();
    }
    ...
    public void requestAccessToken(Callback<OAuthResponse> callback, TwitterAuthToken twitterAuthToken, String str) {
        this.api.getAccessToken(new OAuth1aHeaders().getAuthorizationHeader(getTwitterCore().getAuthConfig(), twitterAuthToken, (String) null, "POST", getAccessTokenUrl(), (Map<String, String>) null), str).enqueue(getCallbackWrapper(callback));
    }

    public void requestTempToken(Callback<OAuthResponse> callback) {
        TwitterAuthConfig authConfig = getTwitterCore().getAuthConfig();
        this.api.getTempToken(new OAuth1aHeaders().getAuthorizationHeader(authConfig, (TwitterAuthToken) null, buildCallbackUrl(authConfig), "POST", getTempTokenUrl(), (Map<String, String>) null)).enqueue(getCallbackWrapper(callback));
    }
}

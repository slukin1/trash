...
public final class UnicastSubject<T> extends Subject<T, T> {
    ...
    public static final class State<T> extends AtomicLong implements Producer, Observer<T>, Observable.OnSubscribe<T>, Subscription {
        ...
        public State(int i11, boolean z11, Action0 action0) {
            Queue<Object> queue2;
            this.terminateOnce = action0 != null ? new AtomicReference<>(action0) : null;
            this.delayError = z11;
            if (i11 > 1) {
                queue2 = UnsafeAccess.isUnsafeAvailable() ? new SpscUnboundedArrayQueue<>(i11) : new SpscUnboundedAtomicArrayQueue<>(i11);
            } else {
        ...
        public boolean checkTerminated(boolean z11, boolean z12, boolean z13, Subscriber<? super T> subscriber2) {
        ...
        public void onNext(T t11) {
            if (!this.done) {
                ...
                try {
                    ...
                    Exceptions.throwOrReport(th2, (Observer<?>) subscriber2, (Object) t11);
                }
            }
        }
    ...
    public static <T> UnicastSubject<T> create(int i11) {
        return new UnicastSubject<>(new State(i11, false, (Action0) null));
    }

    public static <T> UnicastSubject<T> create(boolean z11) {
        return new UnicastSubject<>(new State(16, z11, (Action0) null));
    }

    public static <T> UnicastSubject<T> create(int i11, Action0 action0) {
        return new UnicastSubject<>(new State(i11, false, action0));
    }

    public static <T> UnicastSubject<T> create(int i11, Action0 action0, boolean z11) {
        return new UnicastSubject<>(new State(i11, z11, action0));
    }
}

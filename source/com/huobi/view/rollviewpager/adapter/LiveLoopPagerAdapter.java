...
import com.huobi.view.rollviewpager.LiveRollPagerView;
import com.huobi.view.rollviewpager.hintview.HintView;
import java.util.ArrayList;
import java.util.Iterator;

public abstract class LiveLoopPagerAdapter extends PagerAdapter {
    private ArrayList<View> mViewList = new ArrayList<>();
    private LiveRollPagerView mViewPager;
    private int posDelta;

    public class LoopHintViewDelegate implements LiveRollPagerView.HintViewDelegate {
        private LoopHintViewDelegate() {
        }

        public void initView(int i11, int i12, HintView hintView) {
            if (hintView != null) {
                hintView.initView(LiveLoopPagerAdapter.this.getRealCount(), i12);
            }
        }

        public void setCurrentPosition(int i11, HintView hintView) {
            if (hintView != null && LiveLoopPagerAdapter.this.getRealCount() > 0) {
                hintView.setCurrent(i11 % LiveLoopPagerAdapter.this.getRealCount());
            }
        }
    }

    public LiveLoopPagerAdapter(LiveRollPagerView liveRollPagerView) {
        this.mViewPager = liveRollPagerView;
        liveRollPagerView.setHintViewDelegate(new LoopHintViewDelegate());
    }

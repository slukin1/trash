...
public class MetricsUtil {
    ...
    private void updateLastTimeDifferenceFor(Tag tag, long j11) {
        if (this.ctx.get() == null) {
            Utility.logd("internal.MetricsUtil", "updateLastTimeDifferenceFor: Context is null");
            return;
        }
        SharedPreferences.Editor edit = ((Context) this.ctx.get()).getSharedPreferences("MetricsUtil", 0).edit();
        edit.putLong("time_difference" + tag.getSuffix(), j11).apply();
    }

    public synchronized MetricsUtil getInstance(Context context) {
        MetricsUtil metricsUtil2 = this.metricsUtil;
        if (metricsUtil2 != null) {
            return metricsUtil2;
        }
        MetricsUtil metricsUtil3 = new MetricsUtil(context);
        this.metricsUtil = metricsUtil3;
        return metricsUtil3;
    }

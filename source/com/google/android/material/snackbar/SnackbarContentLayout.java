...
public class SnackbarContentLayout extends LinearLayout implements ContentViewCallback {
    ...
    private static void updateTopBottomPadding(View view, int i11, int i12) {
        if (h0.c0(view)) {
            h0.P0(view, h0.L(view), i11, h0.K(view), i12);
        } else {
            view.setPadding(view.getPaddingLeft(), i11, view.getPaddingRight(), i12);
        }
    }

    private boolean updateViewsWithinLayout(int i11, int i12, int i13) {
        boolean z11;
        ...
        if (this.messageView.getPaddingTop() == i12 && this.messageView.getPaddingBottom() == i13) {
            return z11;
        }
        updateTopBottomPadding(this.messageView, i12, i13);
        return true;
    }
    ...
    public void animateContentIn(int i11, int i12) {
        ...
        long j11 = (long) i12;
        long j12 = (long) i11;
        this.messageView.animate().alpha(1.0f).setDuration(j11).setStartDelay(j12).start();
        if (this.actionView.getVisibility() == 0) {
            this.actionView.setAlpha(0.0f);
            this.actionView.animate().alpha(1.0f).setDuration(j11).setStartDelay(j12).start();
        }
    }
    ...
    public void animateContentOut(int i11, int i12) {
        ...
        long j11 = (long) i12;
        long j12 = (long) i11;
        this.messageView.animate().alpha(0.0f).setDuration(j11).setStartDelay(j12).start();
        if (this.actionView.getVisibility() == 0) {
            this.actionView.setAlpha(1.0f);
            this.actionView.animate().alpha(0.0f).setDuration(j11).setStartDelay(j12).start();
        }
    }
    ...
    public void setMaxInlineActionWidth(int i11) {
        this.maxInlineActionWidth = i11;
    }

    public void updateActionTextColorAlphaIfNeeded(float f11) {
        if (f11 != 1.0f) {
            this.actionView.setTextColor(MaterialColors.layer(MaterialColors.getColor(this, R.attr.colorSurface), this.actionView.getCurrentTextColor(), f11));
        }
    }

...
public final class DashUtil {
    ...
    public static DataSpec buildDataSpec(Representation representation, RangedUri rangedUri, int i11) {
        return new DataSpec.Builder().setUri(rangedUri.resolveUri(representation.baseUrl)).setPosition(rangedUri.start).setLength(rangedUri.length).setKey(representation.getCacheKey()).setFlags(i11).build();
    }
    ...
    private static void loadInitializationData(ChunkExtractor chunkExtractor, DataSource dataSource, Representation representation, boolean z11) throws IOException {
    ...
    private static ChunkExtractor newChunkExtractor(int i11, Format format) {
        ...
        return new BundledChunkExtractor(str != null && (str.startsWith(MimeTypes.VIDEO_WEBM) || str.startsWith(MimeTypes.AUDIO_WEBM)) ? new MatroskaExtractor() : new FragmentedMp4Extractor(), i11, format);
    }
    ...
    private static void loadInitializationData(DataSource dataSource, Representation representation, ChunkExtractor chunkExtractor, RangedUri rangedUri) throws IOException {
        new InitializationChunk(dataSource, buildDataSpec(representation, rangedUri, 0), representation.format, 0, (Object) null, chunkExtractor).load();
    }
}

...
public final class InetAddresses {
    ...
    public static final class TeredoInfo {
        ...
        public TeredoInfo(Inet4Address inet4Address, Inet4Address inet4Address2, int i11, int i12) {
            boolean z11 = true;
            Preconditions.checkArgument(i11 >= 0 && i11 <= 65535, "port '%s' is out of range (0 <= port <= 0xffff)", i11);
            Preconditions.checkArgument((i12 < 0 || i12 > 65535) ? false : z11, "flags '%s' is out of range (0 <= flags <= 0xffff)", i12);
    ...
    public static Inet4Address getCoercedIPv4Address(InetAddress inetAddress) {
        ...
        int asInt = Hashing.murmur3_32().hashLong(j11).asInt() | -536870912;
        if (asInt == -1) {
            asInt = -2;
        }
    ...
    public static TeredoInfo getTeredoInfo(Inet6Address inet6Address) {
        ...
        short readShort = ByteStreams.newDataInput(address, 8).readShort() & Constants.PROTOCOL_NONE;
    ...
    private static byte[] textToNumericFormatV4(String str) {
        ...
        try {
            ...
            for (String parseOctet : IPV4_SPLITTER.split(str)) {
                int i12 = i11 + 1;
                bArr[i11] = parseOctet(parseOctet);
                i11 = i12;
            }
            if (i11 == 4) {
                return bArr;
    ...
    private static byte[] textToNumericFormatV6(String str) {
        ...
        int i13 = -1;
        for (int i14 = 1; i14 < splitToList.size() - 1; i14++) {
            if (splitToList.get(i14).length() == 0) {
                if (i13 >= 0) {
                    return null;
                }
                i13 = i14;
        ...
        if (i13 >= 0) {
            ...
            if (((String) Iterables.getLast(splitToList)).length() == 0 && i12 - 1 != 0) {
                return null;
            }
        } else {
            i11 = splitToList.size();
            i12 = 0;
        }
        int i15 = 8 - (i11 + i12);
        if (i13 < 0 ? i15 != 0 : i15 < 1) {
            return null;
        }
        ...
        while (i16 < i11) {
            try {
                allocate.putShort(parseHextet(splitToList.get(i16)));
                i16++;
            } catch (NumberFormatException unused) {
                return null;
            }
        ...
        while (i12 > 0) {
            allocate.putShort(parseHextet(splitToList.get(splitToList.size() - i12)));
            i12--;
        }

...
class SSOAuthHandler extends AuthHandler {
    ...
    public SSOAuthHandler(TwitterAuthConfig twitterAuthConfig, Callback<TwitterSession> callback, int i11) {
    ...
    private static boolean checkAppSignature(PackageManager packageManager, String str, String str2) {
        try {
            for (Signature charsString : packageManager.getPackageInfo(str, 64).signatures) {
                if (!str2.equals(charsString.toCharsString())) {
                    return false;
                }
            }
            return true;
    ...
    private boolean startAuthActivityForResult(Activity activity) {
        ...
        if (availableSSOPackage == null) {
            Twitter.getLogger().e("Twitter", "SSO app signature check failed", (Throwable) null);
            return false;
        }
        ...
        if (!IntentUtils.isActivityAvailable(activity, component)) {
            Twitter.getLogger().e("Twitter", "SSO auth activity not found", (Throwable) null);
            return false;
        }
        ...
        try {
            ...
            Twitter.getLogger().e("Twitter", "SSO exception occurred", e11);
            return false;
        }
    }

...
public class PerformanceTracker {
    ...
    private final Comparator<c<String, Float>> floatComparator = new Comparator<c<String, Float>>() {
        public int compare(c<String, Float> cVar, c<String, Float> cVar2) {
            float floatValue = ((Float) cVar.f8469b).floatValue();
            float floatValue2 = ((Float) cVar2.f8469b).floatValue();
    ...
    public List<c<String, Float>> getSortedRenderTimes() {
        ...
        for (Map.Entry next : this.layerRenderTimes.entrySet()) {
            arrayList.add(new c((String) next.getKey(), Float.valueOf(((MeanCalculator) next.getValue()).getMean())));
        }
    ...
    public void logRenderTimes() {
        if (this.enabled) {
            ...
            for (int i11 = 0; i11 < sortedRenderTimes.size(); i11++) {
                c cVar = sortedRenderTimes.get(i11);
                Log.d(L.TAG, String.format("\t\t%30s:%.2f", new Object[]{cVar.f8468a, cVar.f8469b}));
            }
        }
    }

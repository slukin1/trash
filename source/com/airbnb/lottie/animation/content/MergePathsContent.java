...
public class MergePathsContent implements PathContent, GreedyContent {
    ...
    private void addPaths() {
        for (int i11 = 0; i11 < this.pathContents.size(); i11++) {
            this.path.addPath(this.pathContents.get(i11).getPath());
        }
    }

    @TargetApi(19)
    private void opFirstPathWithRest(Path.Op op2) {
        ...
        if (pathContent2 instanceof ContentGroup) {
            ...
            for (int i11 = 0; i11 < pathList2.size(); i11++) {
                Path path3 = pathList2.get(i11).getPath();
    ...
    public Path getPath() {
        ...
        int i11 = AnonymousClass1.$SwitchMap$com$airbnb$lottie$model$content$MergePaths$MergePathsMode[this.mergePaths.getMode().ordinal()];
        if (i11 == 1) {
            addPaths();
        } else if (i11 == 2) {
    ...
    public void setContents(List<Content> list, List<Content> list2) {
        for (int i11 = 0; i11 < this.pathContents.size(); i11++) {
            this.pathContents.get(i11).setContents(list, list2);
        }
    }
}

package com.tencent.mm.sdk.openapi;

import android.content.Context;
import android.content.pm.PackageManager;
import android.content.pm.Signature;
import com.huobi.finance.bean.FinanceRecordItem;
import com.tencent.mm.sdk.b.a;

class WXApiImplComm {
    private static final String TAG = "MicroMsg.SDK.WXMsgImplComm";
    private static final String WX_APP_SIGNATURE = "";

    private WXApiImplComm() {
        throw new RuntimeException(getClass().getSimpleName() + " should not be instantiated");
    }

    /* JADX WARNING: Code restructure failed: missing block: B:3:0x0004, code lost:
        r2 = r2.getStringExtra(com.tencent.mm.sdk.constants.ConstantsAPI.Token.WX_TOKEN_KEY);
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public static boolean isIntentFromWx(android.content.Intent r2, java.lang.String r3) {
        /*
            r0 = 0
            if (r2 != 0) goto L_0x0004
            return r0
        L_0x0004:
            java.lang.String r1 = "wx_token_key"
            java.lang.String r2 = r2.getStringExtra(r1)
            if (r2 == 0) goto L_0x0015
            boolean r2 = r2.equals(r3)
            if (r2 != 0) goto L_0x0013
            goto L_0x0015
        L_0x0013:
            r2 = 1
            return r2
        L_0x0015:
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.tencent.mm.sdk.openapi.WXApiImplComm.isIntentFromWx(android.content.Intent, java.lang.String):boolean");
    }

    public static boolean validateAppSignature(Context context, Signature[] signatureArr, boolean z11) {
        String str;
        if (!z11) {
            str = "ignore wechat app signature validation";
        } else {
            int length = signatureArr.length;
            int i11 = 0;
            while (i11 < length) {
                String lowerCase = signatureArr[i11].toCharsString().toLowerCase();
                a.d(TAG, "check signature:" + lowerCase);
                if (lowerCase.equals(WX_APP_SIGNATURE)) {
                    str = FinanceRecordItem.STATE_PASS;
                } else {
                    i11++;
                }
            }
            return false;
        }
        a.d(TAG, str);
        return true;
    }

    public static boolean validateAppSignatureForPackage(Context context, String str, boolean z11) {
        if (!z11) {
            a.d(TAG, "ignore wechat app signature validation");
            return true;
        }
        try {
            return validateAppSignature(context, context.getPackageManager().getPackageInfo(str, 64).signatures, z11);
        } catch (PackageManager.NameNotFoundException unused) {
            return false;
        }
    }
}

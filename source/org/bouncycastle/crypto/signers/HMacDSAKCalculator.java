...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    private BigInteger bitsToInt(byte[] bArr) {
        ...
        return bArr.length * 8 > this.f59311n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f59311n.bitLength()) : bigInteger;
    }

    public void init(BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr) {
        this.f59311n = bigInteger;
    ...
    public BigInteger nextK() {
        int unsignedByteLength = BigIntegers.getUnsignedByteLength(this.f59311n);
        ...
        while (true) {
            ...
            while (i11 < unsignedByteLength) {
                ...
                this.hMac.doFinal(this.V, 0);
                int min = Math.min(unsignedByteLength - i11, this.V.length);
                System.arraycopy(this.V, 0, bArr, i11, min);
                i11 += min;
            }
            BigInteger bitsToInt = bitsToInt(bArr);
            if (bitsToInt.compareTo(ZERO) > 0 && bitsToInt.compareTo(this.f59311n) < 0) {
                return bitsToInt;
            }
            HMac hMac3 = this.hMac;
            byte[] bArr3 = this.V;
